...
export default function Register() {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (isActiveOnChain) setBurnerAddress(activeBurner)
      else setBurnerAddress(null)
    } catch (err) {
  ...
  // ■■■ Execution: App Vault Generation ■■■
  const handleCreateVaultAndRegister = async () => {
    ...
    try {
      ...
      const encryptedKey = await lockAppPrivateKey(rawPrivateKeyHex, vaultPassword)
      ...
      // Auto-chain into Public Key Registry
      await handleJoinTunnel(pubKeyHex)
    } catch (error) {
      console.error('Vault creation failed:', error)
      if (error?.name === 'UserRejectedRequestError') {
        setErrorMsg('Signature rejected. You must sign to generate your secure vault.')
      } else {
        setErrorMsg(error.message || 'Failed to secure vault. Check console for details.')
      }
      setIsActivating(false)
    }
  }

  // ■■■ Execution: Public Key Registry ■■■
  const handleJoinTunnel = async (directPubKeyHex = null) => {
    try {
      ...
      if (!pubKeyHex) throw new Error('No public key found. Please secure local vault first.')
  ...
  const handleAuthorizeSession = async (customPassword = null) => {
    try {
      ...
      let currentKey = localStorage.getItem(chatLocalStorageBurnerKey)

      if (!currentKey || !localStorage.getItem(chatLocalStorageBurnerAddress)) {
        ...
        targetBurnerAddress = localStorage.getItem(chatLocalStorageBurnerAddress)
      }
  ...
  return (
    <div className={clsx(styles.register)}>
      <div className={clsx(styles.register__card)}>
          ...
          >
            ...
            <div className="flex-grow-1">
              ...
              <p>
                {sessionActive
                  ? 'Session mode active'
                  : isSessionOrphaned
                    ? 'Local key missing (Requires Import or Reset)'
                    : 'Activation required'}
              </p>
            ...
            <div
              className={clsx(
                styles.register__status,
                sessionActive
                  ? styles['register__status--ok']
                  : isSessionOrphaned
                    ? styles['register__status--pending']
                    : styles['register__status--pending']
              )}
            >
        ...
        {/* ■■■ Security Modules (Vault, Auth, Decrypt, Import, Backup) ■■■ */}
        <div className={clsx(styles.register__secureModules)}>
          {/* Vault Creation Module */}
          ...
          {showPasswordSetup && (
            <div className={clsx(styles.register__secureSetup)}>
              ...
              <p>Choose a password to encrypt the burner private key on this device.</p>
              <div className={clsx(styles.register__formGroup)}>
                <input
          ...
          {/* Reveal Backup Module */}
          {revealKeyMode && revealedPrivateKey && (
            <div className={clsx(styles.register__secureSetup)}>
              <h5>Backup Session Private Key</h5>
              <p>
                <strong>CAUTION:</strong> Never share this key. Store it securely to recover your session.
              </p>
        ...
        <footer className={clsx(styles.register__footer)}>
          {isFullyRegistered ? (
            ...
            <button className={clsx(styles.register__button, 'btn')} disabled={true}>
          ...
          {!isFullyRegistered && <p className={clsx(styles['register__gas-note'])}>* Gas fee required for each onchain activation step.</p>}
        </footer>
      </div>
    </div>
  )
