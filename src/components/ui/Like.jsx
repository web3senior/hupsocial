...
import { useSidebarStore, getLikeOverride } from '@/stores/useSidebarStore'
 ...
 *
 * Every tap is one transaction — batchLike([id]) or unlike(id) — sent the moment the heart
 * is pressed: sponsored by the relay where the chain allows it, signed silently by an active
 * session key, or confirmed in the wallet. Nothing is queued.
 *
...
export const Like = ({ post, onUpdate }) => {
  ...
  /**
   * Sends one heart action and settles the optimistic state.
    ...
    <div className={clsx('flex', 'align-items-center', 'gap-050')}>
      <Tooltip content={likeLabel} placement="bottom" size="compact" hoverOnly>
        <button
          ...
          disabled={isLoading}
          className={clsx('like-button', isLoading && 'processing')}
          onClick={handleLikeInteraction}
          aria-label={`${likeLabel} post`}
        >
          <HeartIcon
            width={18}
            height={18}
            color={isLiked ? 'var(--liked-color, #ff007a)' : 'currentColor'}
