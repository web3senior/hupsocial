 ...
 * artwork — so the modal over the collection grid and the listing page can each frame it their
 * own way while showing a reader the same thing.
 *
 * Five tabs, because the four questions past "what is it?" are ones a reader asks one at a time.
 * The action card above them never moves: whichever tab is open, buying, listing, transferring
 * and reviewing the top offer stay one tap away.
 *
...
export default function TokenDetailPanel({
  ...
  const tabs = [
    ...
    ...(offersAddress ? [{ key: 'offers', label: 'Offers' }] : []),
    { key: 'activity', label: 'Activity' },
    { key: 'history', label: 'Price History' },
    { key: 'details', label: 'Details' },
  ]

  // Arrow keys move between tabs and open them, per the ARIA tabs pattern; the roving tabindex
  // below keeps a single stop in the page's tab order
  ...
  return (
    <section className={styles.token}>
      ...
      {/* Action card. TradeCard resolves price and status onchain before it will sell anything,
          so a listing indexed a moment ago can never sell on stale terms. */}
      <div className={styles.token__trade}>
        ...
        <div className={styles.token__actions}>
          ...
          {/* The offer itself is accepted from the Offers tab, where the terms and the expiry are
              in view — this is the shortcut to it, not a one-tap accept */}
          {topOffer && (
            <button type="button" className={styles.token__action} onClick={() => setTab('offers')}>
      ...
      <div className={styles.token__tabs} role="tablist" aria-label="Token details" ref={tablistRef} onKeyDown={handleTabKeys}>
        {tabs.map((entry) => (
          <button
            key={entry.key}
            type="button"
            role="tab"
            id={`token-tab-${entry.key}`}
            aria-selected={tab === entry.key}
            aria-controls={`token-panel-${entry.key}`}
            tabIndex={tab === entry.key ? 0 : -1}
            className={clsx(styles.token__tab, tab === entry.key && styles['token__tab--active'])}
      ...
      <div className={styles.token__panel} role="tabpanel" id={`token-panel-${tab}`} aria-labelledby={`token-tab-${tab}`} tabIndex={0}>
        {tab === 'traits' &&
            ...
            <>
              <ul className={styles.token__traits}>
                {metadata.attributes.map((attr) => {
                  ...
                  const floor = stats?.floor ? formatStake(stats.floor, traits.floorCurrency?.decimals) : null
                  ...
                  return (
                    <li key={`${attr.label}:${attr.value}`} className={styles.token__trait}>
                      ...
                      <div className={styles.token__traitFooter}>
                        ...
                        {floor && (
                          <span className={styles.token__traitFloor} title={`Cheapest token with this trait: ${floor} ${floorSymbol}`}>
                            {floor} <small>{floorSymbol}</small>
                          </span>
                        )}
                      </div>
                    </li>
        ...
        {tab === 'offers' && offersAddress && (
          <>
            <OfferList variant="table" chainId={chain} collection={collection} tokenId={tokenId} isLsp8={isLsp8} ownerAddress={owner.owner} />
          ...
          (!priceSeries || priceSeries.points.length === 0 ? (
            ...
            <>
              ...
              {priceSeries.omitted > 0 && (
                <p className={styles.token__note}>
                  {priceSeries.omitted} {priceSeries.omitted === 1 ? 'sale is' : 'sales are'} not plotted — {priceSeries.omitted === 1 ? 'it' : 'they'} settled in
                  another currency, which this axis can&apos;t hold.
                </p>
              )}
            </>
          ))}
